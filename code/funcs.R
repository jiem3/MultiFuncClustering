...
Lambda_and_cluster_old <- function(SIMU_DATA, MIN_LAMBDA = 1, MAX_LAMBDA = 15, STEP_LAMBDA = 1, 
                                   ...
                                   True_cluster = NA )
{
  ...
  for(k in 1:length(K))
  {
    for(lamb in 1:length(Lambda))
    {
      ...
      EM_result <- EM_simple_(SIMU_DATA, K= K[k], Lambda = Lambda[lamb], M = M, method = method, km = km )           
...
Lambda_and_cluster_7_11 <- function(SIMU_DATA, MIN_LAMBDA = 1, MAX_LAMBDA = 15, STEP_LAMBDA = 1, Lambda2 = 10,
                                    K = c(2,3,4), M = 100, method = "individual", km = 2, plot = TRUE,
                                    True_cluster = NA )
{
  options(warn=-1)
  ...
  for(k in 1:length(K))
  {
    for(lamb in 1:length(Lambda))
    {
      ...
      res <- try( EM_result <- EM_simple_old_old_7_11(SIMU_DATA, K= K[k], Lambda = Lambda[lamb], Lambda2 = Lambda2, M = M, method = method, km = km ) )
      if(inherits(res, "try-error"))
      {
        print("There is an error in the EM")
      }
...
Lambda_and_cluster_9_3 <- function(SIMU_DATA, MIN_LAMBDA = 1, MAX_LAMBDA = 15, STEP_LAMBDA = 3, 
                                   MIN_LAMBDA2 = 1, MAX_LAMBDA2 = 10, STEP_LAMBDA2 = 3, 
                                   K = c(2,3,4), M = 100, method = "individual", km = 2, plot = TRUE,
                                   True_cluster = NA )
{
  options(warn=-1)
  ...
  p <- dim(SIMU_DATA)[2]
  Lambda <- seq(from= MIN_LAMBDA, to = MAX_LAMBDA, by = STEP_LAMBDA)
  Lambda2 <- seq(from = MIN_LAMBDA2, to = MAX_LAMBDA2, by = STEP_LAMBDA2)
  Total_Iter <- length(Lambda)*length(Lambda2)
  ...
  # Create a list of Matrix of Results, Converge_status, for each K
  Results_list <- list(Results, Converge_status, Num_zero_mean, BIC, ARI, ZERO_IND)
  names(Results_list) <- c("Results", "Converge_status", "Num_zero_mean", "BIC", "ARI", "ZERO_IND")
  Results_list_K <- list()
  for(i in 1:(length(K)))
  {
    Results_list_K[[i]] <- Results_list
  }
  names(Results_list_K) <- c(paste(K))
  ...
  for(k in 1:length(K))
  {
    for(lamb in 1:length(Lambda))
    {
      for(lamb2 in 1:length(Lambda2))
      {

        res <- try( EM_result <- EM_simple_old_old_7_11(SIMU_DATA, K= K[k], Lambda = Lambda[lamb], Lambda2 = Lambda2[lamb2], M = M, method = method, km = km ) )
        if(inherits(res, "try-error"))
        {
        ...
        if(EM_result$all_zero)
        {
          Results_list_K[[k]][["Results"]][lamb2, lamb] <- 0
          Results_list_K[[k]][["Converge_status"]][lamb2, lamb] <- M - (EM_result$Stop_id + 1) 
          Results_list_K[[k]][["Num_zero_mean"]][lamb2, lamb] <- p*K[k]
          Results_list_K[[k]][["BIC"]][lamb2, lamb] <- 0
          Results_list_K[[k]][["ZERO_IND"]][lamb2, lamb] <- 0
          Results_list_K[[k]][["ARI"]][lamb2, lamb] <- mclust::adjustedRandIndex(EM_result$Cluster , True_cluster)

        }
        else 
        {
          Results_list_K[[k]][["Results"]][lamb2, lamb] <- as.numeric(EM_result$logLik)
          Results_list_K[[k]][["Converge_status"]][lamb2, lamb] <- M - (EM_result$Stop_id + 1) # if Converge status is 0 then means has not converged
          for(m in 1:K[k]){Results_list_K[[k]][["Num_zero_mean"]][lamb2, lamb] <- Results_list_K[[k]][["Num_zero_mean"]][lamb2, lamb]+ length( which(EM_result$MU[[m]][EM_result$Stop_id,]==0)) }
          Results_list_K[[k]][["BIC"]][lamb2, lamb] <- -2*Results_list_K[[k]][["Results"]][lamb2, lamb] + log(N)*( p+K[k]+p*K[k]-1- Results_list_K[[k]][["Num_zero_mean"]][lamb2, lamb] )
          Results_list_K[[k]][["ARI"]][lamb2, lamb] <- mclust:: adjustedRandIndex(EM_result$Cluster , True_cluster)
          Results_list_K[[k]][["ZERO_IND"]][lamb2, lamb]  <- sum(EM_result$zero_prob_ind) # Total number of 0 prob for N*p combinations
        }
      }
    }
    ...
    rownames(Results_list_K[[k]][["Results"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["Results"]]) <- c(paste(Lambda))
    
    rownames(Results_list_K[[k]][["Converge_status"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["Converge_status"]]) <- c(paste(Lambda))
    
    rownames(Results_list_K[[k]][["BIC"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["BIC"]]) <- c(paste(Lambda))
    
    rownames(Results_list_K[[k]][["Num_zero_mean"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["Num_zero_mean"]]) <- c(paste(Lambda))
    
    rownames(Results_list_K[[k]][["ZERO_IND"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["ZERO_IND"]]) <- c(paste(Lambda))
    
    rownames(Results_list_K[[k]][["ARI"]]) <- c(paste(Lambda2))
    colnames(Results_list_K[[k]][["ARI"]]) <- c(paste(Lambda))
  ...
  if(plot==TRUE)
  {
    ...
    for(k in 1:length(K))
    {
      Result <- Results_list_K[[k]][["Results"]]
      
      MIN <- min(  Result[ intersect(which(Result!=0) , which(!is.infinite(Result)) ) ] )
      MAX <- max(  Result[ intersect(which(Result!=0) , which(!is.infinite(Result)) ) ] )
      Result[which(is.infinite(Result))] <- 0
   
      x <-  rep(Lambda2, each = length(Lambda))
      y <-  rep(Lambda, length(Lambda2))
      z <-  as.numeric(t(Result))
      
      s3d <-  scatterplot3d(x, y, z, highlight.3d=TRUE, col.axis="blue", type = "h",angle = 130,
                            col.grid="lightblue", main=paste("Log-likelihood, K=",k), pch=20, zlim = c(MIN, MAX))
      
      for(i in length(Lambda2):1)
        s3d$points3d(rep(Lambda2[i], length(Lambda)), Lambda, Result[i,], type = "l")
      for(i in length(Lambda):1)
        s3d$points3d(Lambda2, rep(Lambda[i], length(Lambda2)), Result[,i], type = "l")
  ...
  if(plot==TRUE)
  {
    ...
    for(k in 1:length(K))
    {
      ...
      s3d <-  scatterplot3d(x, y, z, highlight.3d=TRUE, col.axis="blue", type = "h",angle = 130,
                            col.grid="lightblue", main=paste("BIC, K=",K[k]), pch=20, zlim = c(MIN, MAX))
  ...
  return(list(Result = Results_list_K, Converge = Converge_status, BIC = BIC,Num_zero_mean = Num_zero_mean, 
              Best_Lambda = best_lambda, Best_K = best_K , ZERO_IND = ZERO_IND, ARI = ARI, min_loc = as.numeric(min_bic)))
...
################################ Try to put multivariate functional data into matrix together with 
...
############################################################Generating the 419*(66*M) matrix#################################################################

MFPCA_sensory_old <- function(Mul_Fun_Matrix , M)
{
  #browser()
  L <- dim(Mul_Fun_Matrix)[2]-1
  ...
  if(M >= L)
  {
    print("Number of principal component is larger than dimension of matrix")
  }
  else
  {
    ...
    for( i in 1: N)
    {
      pca_data <- t(Mul_Fun_Matrix[Mul_Fun_Matrix[,L+1]==i,-(L+1)])
      pca_data_fd <- funData(argvals = t, X = pca_data)
      
      pca_fd <- multiFunData(list(pca_data_fd))
      
      # use spline, not working so well
      mfpca <- MFPCA(mFData = pca_fd, M = M, fit = T , 
                     uniExpansions =  list(list(type = "splines1D", k = 5)))
      
      # Slow using uFPCA
      #mfpca <- MFPCA(mFData = pca_fd, M = M, fit = T , 
      #               uniExpansions =   list(list(type = "uFPCA",nbasis = 5 , pve = 0.8)
      #                                     ))
      
      
      Eigenfunctions[[i]] <- mfpca$functions
      EigenValues[i,] <- mfpca$values
      
      MFPCA_results[i, ] <- as.vector(t(mfpca$score))
    }
...
MFPCA_sensory_old2 <- function(Mul_Fun_Matrix , M)
{
  ...
  else
  {
    ...
    for( i in 1: N)
    {
      pca_data <- Mul_Fun_Matrix[Mul_Fun_Matrix[,L+1]==i,-(L+1)]
      #pca_data_fd <- funData(argvals = t, X = pca_data)
      ...
      # use spline, not working so well
      #mfpca<- mfpca(pca_data_fd, nharm = M)
      mfpca<- pca.fd(pca_data_fd, nharm = M)
      
      #Eigenfunctions[[i]] <- mfpca$harmonics
      #EigenValues[i,] <- mfpca$values
      
      MFPCA_results[i, ] <- as.vector(t(mfpca$scores))
    }
...
MFPCA_sensory_old3 <- function(Mul_Fun_Matrix , M)
{
  
  L <- dim(Mul_Fun_Matrix)[2]-1 # Number of sensors
  TT <- dim(Mul_Fun_Matrix[Mul_Fun_Matrix[,L+1]==1,])[1] # Number of measurements (t)
  # Result matrix 
  N <- length(unique(Mul_Fun_Matrix[,L+1])) # number of observations
  p <- L  # number of sensors
  t=1:TT
  
  # Create basis fucntion
  splines <- create.bspline.basis(rangeval=c(1, max(t)), nbasis = 20,norder=4)
  ...
  else
  {
    ...
    for( i in 1: L)
    {
      ...
      # use spline, not working so well
      #mfpca<- mfpca(pca_data_fd, nharm = M)
      mfpca<- pca.fd(pca_data_fd, nharm = M, centerfns = FALSE)
...
###Check PCA component variability##############
