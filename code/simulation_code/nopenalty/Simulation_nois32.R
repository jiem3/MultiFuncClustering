...
if(length(args) > 0) 
  ...
  mu1 <- c(c(14,14,rep(14,9),0), c(11,20,22,rep(23,5),seq(23,18,length.out=4)) ,rep(rep(0,12),32)) 
  ...
  Data_result <- FDATA_GENERATOR(N = N, K =3, P = 34, PC = 12, order= 3, evalRange = c(0,30), MU = MU, SIGMA = SIGMA, prob = c(1/3,1/3,1/3) )

  Data_rescale<-cbind(apply(Data_result$Data[,1:34],2,scale),Data_result$Data[,35])
  ...
  write.csv(result_data, file = paste("result_MB_FDA_nois32_nopara",SEED,".csv", sep = ""))
