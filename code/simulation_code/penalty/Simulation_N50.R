####################################
# Simulation Study: change of sample size
####################################
...
cat("seed is ", SEED, "\n", sep="")
  ...
  set.seed(SEED)
  print(paste("Nopara, Iteration ",SEED," started",sep=""))

  N <- 50 ##sample size
  num_eval <- 31 ## evaluate the functional curve on 31 time points
  

  mu1 <- c(c(14,14,rep(14,9),0), c(11,20,22,rep(23,5),seq(23,18,length.out=4)) ,rep(rep(0,12),16)) 
  ...
  sigma1 <- diag(c(c(20,10,8,rep(2.8,6),rep(2.8,2),c(0.2)), 
                     c(rep(16,11),1), rep(15,12*16) ) )/pgseq  #Variance, corresponding
 
  sigma2 <-diag(c(c(15,15,rep(10,5),seq(5,0.1,length.out=5)),
                    c(rep(30,5),rep(20,6),1),rep(15,12*16) ))     /pgseq

  sigma3 <- diag( c(c(15,rep(12,2),rep(12,2),rep(12,2),rep(12,2),seq(3,0.2,length.out=3)),
                      c(rep(8,12)),rep(15,12*16)) )/pgseq  
 
  MU <- list(mu1, mu2, mu3)
  SIGMA <- list(sigma1, sigma2, sigma3)
  
  
  Data_result <- FDATA_GENERATOR(N = N, K =3, P = 18, PC = 12, order= 3, evalRange = c(0,30), MU = MU, SIGMA = SIGMA, prob = c(1/3,1/3,1/3) )

  Data_rescale<-cbind(apply(Data_result$Data[,1:18],2,scale),Data_result$Data[,19])
      
  FPCA_expansion <- MFPCA_sensory(Data_rescale, M = 3) ##N obs, 3 cols for each sensor of each obs( which is the number of PCA components)


  Lambda_set<-c(0,0.5,1,2,3,5,7,10, 15,20, 25)*(N^(1/3))
  gamma_set<-c(0.5,1,1.5,2)

  print("Set up finished,bigin selection")
  ...
  write.csv(result_data, file = paste("result_MB_FDA_N50_para",SEED,".csv", sep = ""))
