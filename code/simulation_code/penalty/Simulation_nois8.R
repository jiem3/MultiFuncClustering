...
cat("seed is ", SEED, "\n", sep="")
  ...
  mu1 <- c(c(14,14,rep(14,9),0), c(11,20,22,rep(23,5),seq(23,18,length.out=4)) ,rep(rep(0,12),8)) 
  ...
  sigma1 <- diag(c(c(20,10,8,rep(2.8,6),rep(2.8,2),c(0.2)), 
                     c(rep(16,11),1), rep(15,12*8) ) )/pgseq  #Variance, corresponding
 
  sigma2 <-diag(c(c(15,15,rep(10,5),seq(5,0.1,length.out=5)),
                    c(rep(30,5),rep(20,6),1),rep(15,12*8) ))     /pgseq

  sigma3 <- diag( c(c(15,rep(12,2),rep(12,2),rep(12,2),rep(12,2),seq(3,0.2,length.out=3)),
                      c(rep(8,12)),rep(15,12*8)) )/pgseq  

  MU <- list(mu1, mu2, mu3)
  SIGMA <- list(sigma1, sigma2, sigma3)
  
  
  Data_result <- FDATA_GENERATOR(N = N, K =3, P = 10, PC = 12, order= 3, evalRange = c(0,30), MU = MU, SIGMA = SIGMA, prob = c(1/3,1/3,1/3) )
 
  Data_rescale<-cbind(apply(Data_result$Data[,1:10],2,scale),Data_result$Data[,11])
  ...
  write.csv(result_data, file = paste("result_MB_FDA_nois8_para",SEED,".csv", sep = ""))
